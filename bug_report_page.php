...
if( $f_master_bug_id > 0 ) {
	...
	$f_issue_type           = $t_bug->issue_type;
	$f_issue_logged         = $t_bug->issue_logged_date;
	$f_target_resolved      = $t_bug->target_resolved_dt;
	$f_resolved_on          = $t->bug->resolved_on_date;

	$f_resolved_witin       = $t->bug->resolved_witin;
	$f_resolved_via         = $t->bug->resolved_via;
	$f_current_status       = $t->bug->current_status;
	$f_system_impact        = $t->bug->system_impact;
	$f_escalation           = $t->bug->escalation;
	...
	$f_platform				= gpc_get_string( 'platform', '1' );
	...
	// $f_issue_type           = gpc_get_string('issue_type');
	// $f_issue_logged         = gpc_get_string('issue_logged_date');
	// $f_target_resolved      = gpc_get_string('target_resolved_dt');
	// $f_resolved_on          = gpc_get_string('resolved_on_date','');


	$f_current_status      = gpc_get_string('current_status','');
	$f_escalation          = gpc_get_string('escalation','');
	$f_system_impact       = gpc_get_string('system_impact','');
	$f_resolved_via        = gpc_get_string('resolved_via', '');
	$f_resolved_witin      = gpc_get_string('resolved_witin','');
...
$t_show_issue_type = in_array('issue_type', $t_fields);
$t_show_issue_logged = in_array('issue_logged_date', $t_fields);
$t_show_target_resolved = in_array('target_resolved_dt',$t_fields);
$t_show_resolved_on = in_array('resolved_on',$t_fields);

$t_show_current_status = in_array('current_status',$t_fields);
$t_show_escalation = in_array('escalation',$t_fields);
$t_show_system_impact = in_array('system_impact',$t_fields);
$t_show_resolved_witin = in_array("resolved_witin",$t_fields);
$t_show_resolved_via = in_array("resolved_via",$t_fields);
...
<?php
	...
	if($t_show_issue_type){

?>
	
	<tr>
		<th class="category">
			<label for="issue_type"><?php print_documentation_link('issue_type'); ?></label>
		</th>
		<td>
		 <select <?php echo helper_get_tab_index() ?> id="issue_type" name="platform" class="input-sm">
				<option value="client errors">CLient Errors</option>
				<option value="service errors">Service Errors</option>
				<option value="internal errors">Internal errors</option>
			</select>
...
?>
		...
		<td>
		 <select <?php echo helper_get_tab_index() ?> id="issue_type" name="current_status" class="input-sm">
				<option></option>
...
<?php
	...
	if($t_show_escalation){
?>
		<tr>
		<th class="category">
			<label for="current_status">Escalation</label>
		</th>
		<td>
		 <select <?php echo helper_get_tab_index() ?> id="issue_type" name="escalation" class="input-sm">
				<option></option>
...
<?php
	...
	if($t_show_resolved_via){
?>
		<tr>
		<th class="category">
			<label for="current_status">Resolved via</label>
		</th>
		<td>
		<select <?php echo helper_get_tab_index() ?> id="issue_type" name="resolved_via" class="input-sm">
				<option></option>
				<option value="Tv session">Tv session</option>
...
<?php
	...
	if($t_show_resolved_witin){
?>
		<tr>
		<th class="category">
			<label for="current_status">Resolved within</label>
		</th>
		<td>
		<select <?php echo helper_get_tab_index() ?> id="issue_type" name="resolved_witin" class="input-sm">
				<option></option>
...
?>
		<tr>
		<th class="category">
			<label for="issue_type"><?php print_documentation_link('issue_logged_date'); ?></label>
		</th>
		<td>
		<?php echo '<input ' . helper_get_tab_index() . ' type="text" id="due_date" name="os" class="datetimepicker input-sm" ' .
...
?>
	...
	<tr>
		<th class="category">
			<label for="issue_type"><?php print_documentation_link('target_resolved_dt'); ?></label>
		</th>
		<td>
		<?php echo '<input ' . helper_get_tab_index() . ' type="text" id="due_date" name="os_build" class="datetimepicker input-sm" ' .
				'data-picker-locale="' . lang_get_current_datetime_locale() .
				'" data-picker-format="' . config_get( 'datetime_picker_format' ) . '" ' .
				'size="20" maxlength="16" value="' . $t_date_to_display . '" />' ?>
			<i class="fa fa-calendar fa-xlg datetimepicker"></i>
...
<tr>
		<th class="category">
			<label for="issue_type"><?php print_documentation_link('resolved_on'); ?></label>
		</th>
		<td>
		<?php echo '<input ' . helper_get_tab_index() . ' type="text" id="due_date" name="build" class="datetimepicker input-sm" ' .
				'data-picker-locale="' . lang_get_current_datetime_locale() .
				'" data-picker-format="' . config_get( 'datetime_picker_format' ) . '" ' .
				'size="20" maxlength="16" value="' . $t_date_to_display . '" />' ?>
			<i class="fa fa-calendar fa-xlg datetimepicker"></i>
...
<?php event_signal( 'EVENT_REPORT_BUG_FORM', array( $t_project_id ) ) ?>
	<tr>
		<th class="category">
			...
			<p style="color:red;font-size:8px">Add a brief description of any action to be taken to resolved the issue indentify.</p>
		</th>
		<td>
	...
	<tr>
		<th class="category">
			...
			<p style="color:red;font-size:8px">Add a brief description of the issue indentify</p>
		</th>
		<td>
...
<?php
	if($t_show_system_impact){
?>
		<tr>
		<th class="category">
			<label for="current_status">System Impact</label>
			<p style="color:red;font-size:8px">Describe the system impact</p>
		</th>
		<td>
			<textarea class="form-control" <?php echo helper_get_tab_index() ?>  name="system_impact" cols="75" rows="5"><?php echo string_textarea( $f_additional_info ) ?></textarea>
		</td>
		</tr>
<?php } ?>
<?php if( $t_show_additional_info ) { ?>
	<tr>
		<th class="category">
			...
			<p style="color:red;font-size:8px">Describe the business impact relating to this issue.</p>
		</th>
		<td>
