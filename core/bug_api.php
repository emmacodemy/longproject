...
class BugData {
	 ...
	 * issue logged date
	 */
	protected $issue_logged_date = null;

	/**
	 * target resolution date
	 */
	protected $target_resolution_dt= null;
	...
	protected $system_impact = '';

	protected $resolved_witin ='';

	protected $resolved_via ='';

	/**
	 * resolved on date
	 */
	protected $resolved_on_date = null;
	...
	public function __set( $p_name, $p_value ) {
		switch( $p_name ) {
			...
			case 'os':
			case 'os_build': //stand for the target_resolved_date
			case 'build': //stand for the resolved_on
			case 'due_date':
				if( !is_numeric( $p_value ) ) {
					$p_value = strtotime( $p_value );
				}
				break;
			case 'platform'://platform field is used for the issue_type.
	...
	function create() {
		...
		$t_query = 'INSERT INTO {bug}
					    ( project_id,reporter_id, handler_id,duplicate_id,
					      ...
					      version,profile_id, summary, view_state, sponsorship_total, sticky, fixed_in_version,
					      target_version, due_date,issue_type,issue_logged,target_resolved_date,
						  resolved_on,current_status,escalation,system_impact,resolve_witin,resolve_via
					    )
					  VALUES
		...
		db_query( $t_query, array( $this->project_id, $this->reporter_id, $this->handler_id, $this->duplicate_id, $this->priority, $this->severity, $this->reproducibility, $t_status, $this->resolution, $this->projection, $this->category_id, $this->date_submitted, $this->last_updated, $this->eta, $t_text_id,$this->version, $this->profile_id, $this->summary, $this->view_state, $this->sponsorship_total, $this->sticky, $this->fixed_in_version, $this->target_version, $this->due_date,
									$this->platform, $this->os,$this->os_build,$this->build,$this->current_status,$this->escalation,
									$this->system_impact,$this->resolved_witin,$this->resolved_via) );
	...
	function process_mentions() {
		...
		$t_mentioned_user_ids = mention_get_users( $this->current_status );
		$t_all_mentioned_user_ids = array_merge( $t_all_mentioned_user_ids, $t_mentioned_user_ids );

		$t_mentioned_user_ids = mention_get_users( $this->escalation );
		$t_all_mentioned_user_ids = array_merge( $t_all_mentioned_user_ids, $t_mentioned_user_ids );

		$t_mentioned_user_ids = mention_get_users( $this->system_impact);
		$t_all_mentioned_user_ids = array_merge( $t_all_mentioned_user_ids, $t_mentioned_user_ids );

		$t_mentioned_user_ids = mention_get_users( $this->resolved_witin );
		$t_all_mentioned_user_ids = array_merge( $t_all_mentioned_user_ids, $t_mentioned_user_ids );

		$t_mentioned_user_ids = mention_get_users( $this->resolved_via );
		$t_all_mentioned_user_ids = array_merge( $t_all_mentioned_user_ids, $t_mentioned_user_ids );
	...
	function update( $p_update_extended = false, $p_bypass_mail = false ) {
		...
		$t_query = 'UPDATE {bug}
					SET project_id=' . db_param() . ', reporter_id=' . db_param() . ',
						...
						issue_type='. db_param() .',issue_logged='. db_param() .',
						target_resolved_date='. db_param() .',resolved_on='. db_param().',
						current_status ='. db_param() . ',escalation='.db_param().',
						system_impact='. db_param().',resolve_witin='.db_param().',
						resolve_via='.db_param().',';
		...
		$t_fields = array(
			...
			$this->issue_type,$this->issue_logged_date,
			$this->target_resolved_dt,$this->resolved_on_date,
			$this->current_status,$this->escalation,
			$this->system_impact,$this->resolved_witin,
			$this->resolved_via,
		);
		...
		history_log_event_direct( $c_bug_id, 'issue_type',$t_old_data->issue_type,$this->issue_type );
		history_log_event_direct( $c_bug_id, 'issue_logged_date', $t_old_data->issue_logged_date,$this->issue_logged_date );
		history_log_event_direct( $c_bug_id, 'target_resolved_date', $t_old_data->target_resolved_dt,$this->target_resolved_dt );
		history_log_event_direct( $c_bug_id, 'resolved_on', $t_old_data->resolved_on_date,$this->resolved_on_date );
		history_log_event_direct( $c_bug_id, 'resolved_witin', $t_old_data->resolved_witin,$this->resolved_witin );
		history_log_event_direct( $c_bug_id, 'resolved_via', $t_old_data->resolved_via,$this->resolved_via );
		history_log_event_direct( $c_bug_id, 'current_status', $t_old_data->current_status,$this->current_status );
		history_log_event_direct( $c_bug_id, 'escalation', $t_old_data->escalation,$this->escalation );
		history_log_event_direct( $c_bug_id, 'system_impact', $t_old_data->system_impact,$this->system_impact );
...
function bug_set_field( $p_bug_id, $p_field_name, $p_value ) {
	...
	switch( $p_field_name ) {
		...
		case 'last_updated':
		case 'date_submitted':
		case 'issue_logged_date':
		case 'target_resolved_dt':
		case 'resolved_on_date':
		case 'due_date':
			if( !is_numeric( $p_value ) ) {
